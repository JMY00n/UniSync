...
<title>공지사항 수정</title>
<style>
    /* writeNoticeForm.jsp와 스타일 동일하게 사용 */
...
<div class="write_box">
    <h2 style="border-bottom: 2px solid #516e7f; padding-bottom: 10px;">✏️ 공지사항 수정</h2>
    ...
    <form action="updateNoticePro.jsp" method="post" enctype="multipart/form-data">
        ...
        <div class="input_row">
            <label>내용</label>
            <textarea name="content" rows="12" required><%= msg.getContent() %></textarea>
        </div>
        
        <div class="input_row">
            <label>새로운 첨부파일 (기존 파일: <%= (msg.getFile_path() != null) ? msg.getFile_path() : "없음" %>)</label>
            <input type="file" name="uploadFile">
            <p style="font-size: 12px; color: gray; margin-top: 5px;">※ 새로운 파일을 선택하면 기존 파일이 교체됩니다. 선택하지 않으면 기존 파일이 유지됩니다.</p>
        </div>
