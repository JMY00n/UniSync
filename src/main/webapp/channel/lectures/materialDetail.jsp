...
<%
    ...
    // 등록자 표시명: 닉네임(name) 우선, 없으면 학번(user_id)으로 폴백
...
<style>
  ...
  .d-top { border-top: 3px solid #1D9E75; }
  ...
  .d-file a { color: #178a65; text-decoration: none; font-weight: 600; }
  ...
  .btn-list { background: #1D9E75; color: #fff; border-color: #1D9E75; }
  .btn-list:hover { background: #178a65; border-color: #178a65; box-shadow: 0 4px 14px rgba(29,158,117,.28); }
  ...
  <div class="d-card">
    ...
    <div class="d-actions">
      <% if(userId.equals(msg.getUser_id())) { %>
        <button type="button" class="btn-edit" onclick="location.href='updateMaterialForm.jsp?message_id=<%= message_id %>&channel_id=<%= channel_id %>'">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
          수정
        </button>
        <button type="button" class="btn-del" onclick="if(confirm('이 강의자료를 정말 삭제하시겠습니까?')) location.href='deleteMaterialPro.jsp?message_id=<%= message_id %>&channel_id=<%= channel_id %>&menu=material'">
      ...
      <button type="button" class="btn-list" onclick="location.href='../lectures/lectureMain.jsp?channel_id=<%= channel_id %>&menu=material'">
    ...
    <div class="d-nav">
      <% if(prev != null) { %>
        <a class="d-nav-row link" href="materialDetail.jsp?message_id=<%= prev.getMessage_id() %>&channel_id=<%= channel_id %>">
      ...
      <% } %>
      <% if(next != null) { %>
        <a class="d-nav-row link" href="materialDetail.jsp?message_id=<%= next.getMessage_id() %>&channel_id=<%= channel_id %>">
